...
def exam_student_view(request, olympiad_id):
    ...
    if olympiad.is_active():
        ...
        if olympiad:
            ...
            for problem in problems:
                Result.objects.get_or_create(contestant_id=contestant_id,olympiad_id=olympiad_id,problem_id=problem.id)

        results = Result.objects.filter(contestant_id=contestant_id,olympiad_id=olympiad_id).order_by('problem__order')

        return render(request, 'olympiad/exam.html', {'results': results, 'olympiad': olympiad, 'contestant': contestant})
...
def student_supplement_view(request, olympiad_id):
    ...
    if olympiad.is_grading:
        ...
        return render(request, 'olympiad/supplement_exam.html', {'results': results, 'olympiad': olympiad, 'contestant': contestant})
    else:
...
def exam_grading_view(request,problem_id):
    ...
    results = Result.objects.filter(problem_id=problem_id,contestant__data__province__isnull=False).order_by('score').reverse
...
def zone_exam_grading_view(request,problem_id,zone_id):
    ...
    if z_id:
        results = Result.objects.filter(problem_id=problem_id,contestant__data__province__zone_id=z_id).order_by('score').reverse
    else:
        results = Result.objects.filter(problem_id=problem_id).order_by('score').reverse

    return render(request, 'olympiad/zone_exam_grading.html', {'results': results, 'problem': problem, 'zone_id': zone_id})
    ...
    if result_id > 0:
        ...
        if request.method == 'POST':
            ...
            if form.is_valid() and request.user.is_staff:
                ...
                result.state = 2
                result.save()
                url = reverse('zone_olympiad_exam_grading',kwargs={'problem_id': result.problem.id, 'zone_id': zone_id})
                url = url + '#result{}'.format(result.id)
                return redirect(url)
        form = ResultsGraderForm(instance=result)
...
def student_exam_materials_view(request):
    ...
    title = "ID: {}, {}, {} сурагчийн илгээсэн материалууд".format(request.user.id,request.user.first_name,request.user.last_name)
